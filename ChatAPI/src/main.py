...
# WOLFS ONLY EXIST TO FEAR YOU
# STOP BEING FEARED BY THEM
# AND THE WOLFS WILL NOT EXIST
# FEAR IS THE ENEMY NOT THE WOLF!
...
async def horoscope_chat(websocket: websockets.WebSocket):
	...
	ai_reply = "👋 Welcome, how may I help you today?"
	...
	try:
		...
		while True:  # beat forever....my sweet love
			...
			# I CAN WRITE CODE WITHOUTH MIRTH
			# THEN, WHAT HAVE I BECOME?
			# A MACHINE BUILDING ANOTHER MACHINE!
			# 
			...
			if not is_followup and all_checks_pass:  # only call LLM if all checks and balances are even
				...
				user_question = input_parsed["text"]
				topic_text = input_parsed["topic"]
				topic_text = input_parsed["topic"]
				is_include_response = "teacher_mode" in input_parsed["command"]
				...
				input_parsed["text_html"] = ChatTools.highlight_relevant_keywords_llm(question_text=user_question, answer_text=ai_reply)
				input_parsed["followup_questions"] = ChatTools.generate_followup_questions_llm(keywords_text=user_question, main_text=ai_reply)
				packed_box = ChatTools.package_reply_for_shippment(**input_parsed)  #pass the same args back in
	...
	# Handle failed gracefully
		...
		print(e)
		raise  # rethrows the last exception
...
async def horoscope_regenerate_embeddings(payload: RegenPayload):
	...
	prediction_nodes = ChatTools.vedastro_predictions_to_llama_index_documents(horoscopeDataList)  #TODO update to using llama-index loader
