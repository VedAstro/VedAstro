...
class AstroTable {
  ...
  async GenerateTable(userInputParams) {
    ...
    $(`#${this.ElementID}`).append(
      `<table id="${this.TableId}" class="table table-striped table-hover table-bordered text-nowrap" style="width: 100%"></table>`
    );
...
window.GenerateAshtakvargaTable = (settings, inputArguments) => {
  // Initialize astro table
  var planetDataTable = new AshtakvargaTable(settings);
  // Generate table
  planetDataTable.GenerateTable(inputArguments);
};
