...
`;
    ...
    $(document).on("onChangeSelectedPerson", function (e) {
      // show message to user that location was found and set
      Swal.fire({
        icon: "success",
        title: "Horoscope selected!",
        html: `AI Astrologer will use this horoscope as reference.`,
        timer: 3000,
        showConfirmButton: false,
      });
    });
  ...
  onmessage(event) {
    ...
    $("#ChatWindowMessageList li").eq(-2).after(aiInputChatCloud);
  ...
  async OnClickSendChat() {
    ...
    $("#ChatWindowMessageList li").eq(-2).after(userInputChatCloud);
