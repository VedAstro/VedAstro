...
namespace VedAstro.Library
{
    ...
    /// </summary>
    public class MLTableRow(PlanetName selectedPlanet, Time time, List<APIFunctionResult> dataRowList)
    {
        ...
        public static string ListToCSV(List<MLTableRow> tableRowList)
        {
            ...
            PropertyInfo[] properties = typeof(MLTableRow).GetProperties().Where(p => p.Name != "DataRowList").ToArray();
            
            //names of the columns with custom name combined with param EXP: IsPlanetBenefic+Sun
            var columnNames = tableRowList[0].DataRowList.Select(d => d.Name);
            ...
            foreach (var row in tableRowList)
            {
                ...
                rowValues.AddRange(row.DataRowList.Select(d => Tools.QuoteValue(d.ResultAsString())));
