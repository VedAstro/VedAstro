    ...
    public static class Krishnamurti
    {
        ...
        public static Dictionary<HouseName, Angle> AllHouseCuspLongitudes(Time time, int horaryNumber = 0, double rotateDegrees = 0)
        {
            ...
            static Dictionary<HouseName, Angle> AllHouseCuspLongitudesHorary(Time time, int horaryNumber)
            {
                ...
                var armc = Calculate.AscendantDegreesToARMC(tropAsc, eps, location.Latitude(), time);
            ...
            static Dictionary<HouseName, Angle> AllHouseCuspLongitudesRotateKundali(Time time, double rotateDegrees)
            {
                ...
                var armc = Calculate.AscendantDegreesToARMC(rotateDegrees, eps, location.Latitude(), time);
            ...
            static Dictionary<HouseName, Angle> AllHouseCuspLongitudesKundali(Time time)
            {
                ...
                swissEph.swe_houses_ex(julDayUt, iFlag, location.Latitude(), location.Longitude(), 'P', cusps, ascmc);
        ...
        public static double HoraryNumberToSiderealAscendant(int horaryNumber)
        {
            ...
            foreach (var constellation in allConstellations)
            {
                ...
                foreach (var planetName in planetnameArrayB)
                {
                    ...
                    {
                        zSignAtLongBefore = Calculate.ZodiacSignAtLongitude(Angle.FromDegrees(siderealAsc));
                        constellationBefore = Calculate.ConstellationAtLongitude(Angle.FromDegrees(siderealAsc));
                        constellationLordBefore = Calculate.LordOfConstellation(constellationBefore.GetConstellationName());
                    }
                    ...
                    {
                        var remainderFromDivBy30 = (siderealAsc % 30.00); //past sign change degree amount
