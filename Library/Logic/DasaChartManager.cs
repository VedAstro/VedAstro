...
namespace VedAstro.Library
{
    ...
    /// </summary>
    public static class DasaChartManager
    {
        ...
        /// </summary>
        public static async Task<string> GenerateDasaChartSvg(DasaChart chartSpecs)
        {
        ...
        /// </summary>
        public static async Task<DasaChart> GenerateDasaChart(Person foundPerson, TimeRange timeRange, double daysPerPixel, List<EventTag> eventTags, ChartOptions summaryOptions)
        {
            //a new chart is born
            var newChartId = Tools.GenerateId();
            var newChart = new DasaChart(newChartId, "", foundPerson, timeRange, daysPerPixel, eventTags, summaryOptions);

            //from person get SVG chart as string
            var eventsChartSvgString = await GenerateDasaChartSvg(newChart);
        ...
        private static string GetJsCodeSvg(string randomId)
        {
            ...
            jsCode += "<script href=\"https://www.vedastro.org/js/DasaChartClass.js\" />";
            ...
            jsCode += $@"
	                    <script type=""text/ecmascript"">
		                    <![CDATA[
                               ...
                               //START THE BEAST
		                       var index = new DasaChart(""{randomId}"");
			                ]]>
	                    </script>
                            ";
        ...
        private static string GenerateEventRows(double hoursPerPixel, Person inputPerson, List<Time> timeSlices, ChartOptions options, ref int yAxis)
        {
            ...
            var eventList = DasaChartManager.UnsortedEventList.OrderByDescending(x => x.DurationMin).ToList();
